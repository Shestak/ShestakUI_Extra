...
----------------------------------------------------------------------------------------
--	Archaeology(stArchaeologist by Safturento)
----------------------------------------------------------------------------------------
stArch = {}
...
function stArch:OnLoad(self)
	...
	archSkill['frame']:SetPoint("TOP", progressBars['frame'], "BOTTOM", 0, -5)
	...
	archSkill['bar']:SetScript("OnMouseUp", function()
		if IsAddOnLoaded("Blizzard_ArchaeologyUI") then
			...
			local loaded, _ = LoadAddOn("Blizzard_ArchaeologyUI")
			if loaded then
	...
	progressBars['solveToggle']:SetScript("OnMouseUp", function()
		if progressBars['solveFrame']:IsShown() then
			...
			for i = 1, 9 do progressBars[i]['solve']:Show() end
...
function stArch:updateSkillBar()
	...
	if (skill['rank'] + 5) > skill['maxRank'] and skill['maxRank'] ~= 525 then
		...
		for c = 0, 4 do
			for s = 1, GetContainerNumSlots(c) do
				...
				if slotID == artifact['keyID'] then
					local _, count = GetContainerItemInfo(c,s)
	...
	button:SetScript("OnClick", function()
		...
		if GetNumArtifactsByRace(index) > 0 then
			if stArch['artifactInfo'][index]['canSolve'] then 
				...
				if (stArch['archSkill']['rank'] + 5) > stArch['archSkill']['maxRank'] and stArch['archSkill']['maxRank'] ~= 525 and not IsShiftKeyDown() then
